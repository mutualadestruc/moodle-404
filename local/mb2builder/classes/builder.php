...
if (!class_exists('mb2builderBuilder')) {
    ...
    class mb2builderBuilder {
        ...
        public static function check_for_generico($str) {
            ...
            if (!$DB->record_exists_sql($sql, ['generico', 1])) {
                return $str;
            }
        ...
        public static function check_urltolink_filter() {
            ...
            // If not - return true.
            if (!$DB->record_exists_sql($sql, ['urltolink', 1])) {
                return true;
            }

            // Urltolink filter is enabled, so we have to check oreding of the filters.
            $mb2shortcodes = $DB->get_record_sql( $sql, ['mb2shortcodes', 1]);
            $urltolink = $DB->get_record_sql( $sql, ['urltolink', 1]);
        ...
        public static function page_settings_form() {
            ...
            $output .= '<button type="button" class="btn btn-sm btn-success" data-modal="#mb2-pb-modal-page-settings">' .
            get_string('save') . '</button>';
            $output .= '<button type="button" class="btn btn-sm btn-danger" data-modal="#mb2-pb-modal-page-settings">' .
