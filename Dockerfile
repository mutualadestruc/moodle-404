...
RUN apt-get update && \
  ...
  libonig-dev libcurl4-openssl-dev libjpeg-dev libfreetype6-dev locales \
  && rm -rf /var/lib/apt/lists/*

# Configurar locales para inglés y español
RUN sed -i -e '/en_US.UTF-8/s/^# //' -e '/es_ES.UTF-8/s/^# //' -e '/en_AU.UTF-8/s/^# //' /etc/locale.gen && locale-gen en_US.UTF-8 es_ES.UTF-8 en_AU.UTF-8
